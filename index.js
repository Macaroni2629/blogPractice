...
app.post("/createPost", function (request, response) {
  ...
  const sql = `INSERT INTO Posts (title, subtitle, body) VALUES (($1), ($2), ($3));`;
  const sqlParameters = [request.body.title, request.body.subtitle, request.body.body];
...
app.get("/viewPost/:post_id", function (request, response) {
  ...
  const sqlParameters = [request.params.post_id];
...
app.post("/editPost/:post_id", function (request, response) {
  ...
  const sqlParameters = [request.body.title, request.body.subtitle, request.body.body, request.params.post_id];
  ...
  pool.query(sql, sqlParameters, function(error) {
    if (error) {
      ...
      response.redirect(`/viewPost/${request.params.post_id}`);
    }
  });
});
